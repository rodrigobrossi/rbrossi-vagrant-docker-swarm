...
Vagrant.configure("2") do |config|
    ...
    config.vm.define "manager" do |i|
      ...
      if auto 
        ...
        --replicas 1 \
        --name cockroachdb-manager \
        --hostname cockroachdb-manager \
        --network cockroachdb \
        --mount type=volume,source=cockroachdb-manager,target=/cockroach/cockroach-data-manager,volume-driver=local \
        --stop-grace-period 60s \
        --publish 8080:8080 \
        cockroachdb/cockroach:v2.0.6 start \
        --join=cockroachdb-manager:26257,cockroachdb-worker1:26257,cockroachdb-worker2:26257 \
        --cache=.25 \
        --max-sql-memory=.25 \
        --logtostderr \
        --insecure"
        ...
        i.vm.provision "shell", inline: "sudo docker run --rm --network=cockroachdb cockroachdb/cockroach:v2.0.6 init --host=cockroachdb-manager --insecure" 
  ...
  instances.each do |instance| 
    config.vm.define instance[:name] do |i|
      ...
      if auto
        ...
        i.vm.provision "shell", inline: "echo Creating worker instance[:name] "
        i.vm.provision "shell", inline: "sudo docker service create \
          --replicas 1 \
          --name cockroachdb-#{instance[:name]} \
          --hostname cockroachdb-#{instance[:name]} \
          --network cockroachdb \
          --mount type=volume,source=cockroachdb-#{instance[:name]},target=/cockroach/cockroach-data-#{instance[:name]},volume-driver=local \
          --stop-grace-period 60s \
          cockroachdb/cockroach:v2.0.6 start \
          --join=cockroachdb-manager:26257,cockroachdb-worker1:26257,cockroachdb-worker2:26257 \
          --cache=.25 \
          ...
          --logtostderr \
          --insecure"
        i.vm.provision "shell", inline: "docker node promote #{instance[:name]}"
      end
    end 
